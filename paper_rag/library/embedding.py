...
class EmbeddingError(RuntimeError):
    ...
    def __init__(self, message: str, code: str = "embedding_unavailable"):
    ...
    def embed_texts(self, texts: list[str]) -> list[list[float]]:
        ...
        try:
            ...
            try:
                ...
                code = str(
                    (detail.get("error") or {}).get("code") or "embedding_http_error"
                )
            except (ValueError, AttributeError, UnicodeError):
                code = "embedding_http_error"
            if len(code) > 80 or not code.replace("_", "").replace("-", "").isalnum():
                code = "embedding_http_error"
            raise EmbeddingError(f"Embedding HTTP {exc.code} ({code})", code) from exc
...
def parse_embedding_response(
    ...
    if [index for index, _ in indexed] != list(range(expected_count)):
        raise EmbeddingError(
            "Embedding indices must cover each input exactly once", "embedding_invalid"
        )
    vectors = [vector for _, vector in indexed]
    if len(vectors) != expected_count:
        raise EmbeddingError(
            f"预期 {expected_count} 个 embedding，实际得到 {len(vectors)} 个"
        )
    return vectors
