...
def main() -> int:
    ...
    try:
        ...
        if args.validated:
            print(json.dumps({"original_query": original_query, "metadata": validate_metadata_parse(strip_code_fence(content))}, ensure_ascii=False, indent=2))
        else:
...
def parse_once(client: PlanParserClient, original_query: str) -> str:
    return client.complete_json(metadata_parser_system_prompt(), original_query)
