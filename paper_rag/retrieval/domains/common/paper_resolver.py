...
def resolve_parser_papers(
    ...
    *,
    ...
    filters, filter_matches, filter_papers = normalize_title_filters(settings, parser_result.get("filters") or [])
    ...
    resolved_papers = merge_paper_lists(flatten_resolved_papers(resolved_anchor_papers), filter_papers)
    if fallback_query and not resolved_papers:
        fallback_papers, fallback_matches = resolve_paper_queries(settings, [fallback_query])
        resolved_papers = merge_paper_lists(resolved_papers, fallback_papers)
...
def normalize_title_filters(settings: Settings, filters: list[dict[str, Any]]) -> tuple[list[dict[str, Any]], list[AliasMatch], list[dict[str, Any]]]:
    ...
    for filter_item in filters:
        if filter_item.get("field") == "title" and filter_item.get("op") in {"=", "in"}:
            values, matches, papers = canonical_title_values(settings, flatten_filter_value(filter_item.get("value")))
            exact_title_values.setdefault(bool(filter_item.get("negated")), []).extend(values)
            alias_matches.extend(matches)
            if not filter_item.get("negated"):
                resolved_papers = merge_paper_lists(resolved_papers, papers)
        else:
...
def canonical_title_values(settings: Settings, values: list[str]) -> tuple[list[str], list[AliasMatch], list[dict[str, Any]]]:
    titles: list[str] = []
    alias_matches: list[AliasMatch] = []
    resolved_papers: list[dict[str, Any]] = []
    for value in values:
...
def flatten_resolved_papers(resolved_groups: dict[str, list[dict[str, Any]]]) -> list[dict[str, Any]]:
    return merge_paper_lists(*resolved_groups.values())


def merge_paper_lists(*paper_lists: list[dict[str, Any]]) -> list[dict[str, Any]]:
    seen: set[str] = set()
