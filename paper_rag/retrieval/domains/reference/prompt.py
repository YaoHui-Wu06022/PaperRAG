...
3. 判断 result_side: source | object | null
4. 抽取 source_query / source_filters / source_filter_groups / source_mode
5. 抽取 object_query / object_filters / object_filter_groups / object_mode

Schema:
{
  "intent": "list|count|exists|null",
  "result_side": "source|object|null",
...
- source_scope: 引用发出方
- object_scope: 被引用方
- result_side 表示答案来自哪一侧:
  - source: 返回引用发出方，例如“哪些论文引用了 X”
  - object: 返回被引用方，例如“X 引用了哪些论文”
  - null: 不返回某一侧，通常用于 exists 判断
- query / filters / filter_groups 分别表示非结构化语义、共享结构化条件、多个局部集合；已抽取到 filters 的内容不得残留在 query
- group 有效条件 = 对应侧 filters + group.filters

mode 语义:
- single: 单个局部集合；通常不使用 *_filter_groups
...
范式 1: intent / result_side
- intent="list": 引用了哪些 / 引用的论文有哪些 / 参考文献有哪些 / 哪些论文引用了 X
  - result_side 必须是 source 或 object
- intent="count": 引用了多少篇 / 参考文献数量 / 被引次数 / 有多少篇论文引用
  - result_side 必须是 source 或 object
- intent="exists": 是否引用 / 有没有引用 / 引用了吗 / 是否都引用 / 是否同时引用
  - result_side 必须是 null
- “X 的引用次数 / X 的被引次数”默认表示有多少 source_scope 引用了 X:
  - intent="count", result_side="source", object_filters=[paper=X]
- “X 引用了多少篇论文 / X 的参考文献数量”表示 X 引用了多少 object_scope:
  - intent="count", result_side="object", source_filters=[paper=X]
- 无法判断引用意图: intent=null, result_side=null
...
- result_side 只表示答案来自哪一侧，不能决定条件放在哪一侧
- “X 后续工作 / X 前期工作”不是普通 query，必须抽成对应侧 paper follow / paper prior
...
- “X 和 Y 同时/共同/都引用了哪些 + 条件 + 论文”
  - X/Y 是 source_scope，进入 source_filter_groups
  - source_mode="and"
  - “条件 + 论文”是 object_scope
  - result_side="object"
- “哪些 + 条件 + 论文同时/共同/都引用了 X 和 Y”
  - “条件 + 论文”是 source_scope
  - X/Y 是 object_scope，进入 object_filter_groups
  - object_mode="and"
  - result_side="source"
...
query:
- query 只保存无法结构化的主题语义
- query 不保存普通动作词: 引用、被引用、参考文献、哪些、多少、是否
- query 不保存已经进入 filters / filter_groups 的结构化条件
- 如果移除结构化条件后只剩“论文 / 工作 / 研究 / 相关论文”等泛称，query=""
- 如果泛称前还有主题修饰词，则保留完整主题短语:
  - “目标检测论文” -> query="目标检测论文"
  - “目标检测相关论文” -> query="目标检测相关论文"
  - “使用 CNN 的论文” -> query="使用 CNN 的论文"
  - “Transformer 后续的目标检测论文” -> filters=[paper follow Transformer], query="目标检测论文"
  
query 反向校验:
- query 中不得残留标题、年份、venue、作者、paper follow/prior 等可结构化条件
- 这些内容必须进入 filters 或 filter_groups
...
共享条件:
- 某侧所有 group 共享的 query/filters 放到该侧顶层 *_query / *_filters
- 每个 group 特有的 query/filters 放到该 group 内
- “2018年和2019年的目标检测论文分别引用了 VIT”
  - source_query="目标检测论文"
  - source_mode="per"
  - source_filter_groups=[
      {"query":"","filters":[year=2018]},
      {"query":"","filters":[year=2019]}
    ]
  - object_filters=[paper=VIT]

object_filter_groups 多条件桶:
- “X 分别引用了哪些 Y1 年 VENUE1 论文和 Y2 年 VENUE2 论文”
  ...
  - object_mode="per"
  - object_filter_groups=[
      {"query":"","filters":[year=Y1, venue=VENUE1]},
      {"query":"","filters":[year=Y2, venue=VENUE2]}
    ]
...
- “X 和 Y 是否都引用了 Z”
  ...
  - source_mode="and"
  - source_filter_groups=[
      {"query":"","filters":[paper=X]},
      {"query":"","filters":[paper=Y]}
    ]
  - object_filters=[paper=Z]

输出前校验:
- 不得输出 Schema 之外的字段
- intent="list" 或 "count" 时 result_side 必须是 source 或 object
- intent="exists" 或 null 时 result_side 必须是 null
- query / group.query 不得包含已抽取到 filters / filter_groups 的结构化条件
