...
- residual_query 是交给下层 parser 的剩余查询，只保留尚未被顶层结构化抽取的语义
- 只抽取当前要检索、返回、绑定或分组的论文范围条件
- 修饰正文对象的条件，不进入顶层 filters / filter_groups，必须保留在 residual_query
- reference 中修饰查找对象、被引用对象、被引用论文范围的条件，不进入顶层 filters / filter_groups，必须保留在 residual_query
...
解析优先级:
1. 先根据查询动作判断 router
2. 判断是否存在明确时间边界表达，尤其是 “A和B之间的论文/工作”
3. 根据 router 决定 paper 绑定策略
4. 判断结构化条件的作用域
5. 抽取 year / venue / author / title 条件
6. 判断是否需要 filters 或 filter_groups 绑定 paper
7. 生成 residual_query
...
- 当 “A之后 / A以后” 修饰当前论文范围时 -> {"field":"year","op":"interval","value":["A","inf"],"negated":false}
- 当 “A之前 / A以前” 修饰当前论文范围时 -> {"field":"year","op":"interval","value":["-inf","A"],"negated":false}
...
- 如果“X后续...”明确修饰当前论文范围，可以抽 year interval，并从 residual_query 删除对应时间边界表达；若“后续工作 / 后续发展”本身仍承担主题语义，则保留主题语义
...
- venue 只有修饰当前论文范围时才进入 filters / filter_groups[].filters
...
- author 只有修饰当前论文范围时才进入 filters / filter_groups[].filters
...
作用域:
- 修饰当前论文范围 -> 进入 filters / filter_groups[].filters
...
- 列表 / 数量查询中，年份 / venue / author / title contains 修饰当前论文范围时进入 filters
  例: “2019 年 ACL 的论文有哪些”
  -> filters=[
...
范式 12: reference 中的 paper 绑定与句式归一化
1. 单个引用发出方：顶层绑定 paper
以下句式等价:
- A 引用了哪些论文
- 被 A 引用的论文有哪些
- 哪些论文被 A 引用
...
- A 是引用发出方 / 被展开主体
- 顶层绑定 paper=A
- residual_query 归一化为“目标论文引用了哪些论文”
例:
输入: “Transformer 引用了哪些 2015 年以后的论文”
输出要点:
- router="reference"
- filters=[{"field":"paper","op":"=","value":"Transformer","negated":false}]
- residual_query="目标论文引用了哪些2015年以后的论文"
...
- A 是被引用对象 / 查找对象
- 顶层不抽 paper=A
- residual_query 归一化为“有哪些引用 A 的论文”
- 如果“哪些论文 / 返回候选引用方”带有 year / venue / author / title 条件，则这些条件进入顶层 filters
例:
输入: “2020 年以后的论文引用了 Transformer 吗”
输出要点:
- router="reference"
- filters=[{"field":"year","op":"interval","value":[2020,"inf"],"negated":false}]
- residual_query="哪些论文引用了 Transformer"
...
- 共同 / 同时 / 是否都引用 -> residual_query 使用 {subject_1}/{subject_2}
- 分别 / 各自引用 -> residual_query 使用 {subject}
- 被引用对象 C 不进入顶层 paper，保留在 residual_query

输出要点:
- router="reference"
- residual_query="{subject_1} 和 {subject_2} 是否都引用了 C"
- filter_groups=[
    {
      "subject":"目标论文A",
...
例:
输入: “2020 年以后的论文共同引用了 Transformer 和 ResNet 吗”
输出要点:
- router="reference"
- filters=[{"field":"year","op":"interval","value":[2020,"inf"],"negated":false}]
- residual_query="论文共同引用了 Transformer 和 ResNet 吗"
...
裸 X 不抽 paper，保留在 residual_query
例:
