...
- 用统一的检索链路支撑命令行、Streamlit UI 和 Colab Public Paper Benchmark
...
- Colab Notebook + Public Paper Benchmark
...
项目保留了一套独立的 Colab 检索测评链路，只评估公开论文数据集上的召回质量，不涉及 LLM 生成。

- Notebook: [`notebooks/PaperRAG_Public_Paper_Eval_Colab.ipynb`](notebooks/PaperRAG_Public_Paper_Eval_Colab.ipynb)
- Benchmark: [`colab_eval/public_paper_benchmark.py`](colab_eval/public_paper_benchmark.py)

这套测评会统一输出 Qasper / SciFact 的 summary、detail 和 failure 明细，方便做参数 A/B 对比。
...
### Colab Public Paper Benchmark

- Notebook: [`notebooks/PaperRAG_Public_Paper_Eval_Colab.ipynb`](notebooks/PaperRAG_Public_Paper_Eval_Colab.ipynb)
- Benchmark 实现: [`colab_eval/public_paper_benchmark.py`](colab_eval/public_paper_benchmark.py)

## 项目结构

```text
app/            Streamlit 前端
colab_eval/     Colab 公开数据集检索测评
...
notebooks/      Colab Notebook 与评测产物
...
- 额外建设了 Colab Public Paper Benchmark，用 Qasper / SciFact 做公开论文检索评测
