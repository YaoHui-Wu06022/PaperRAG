...
class IngestMetadataLookupTests(unittest.TestCase):
    def test_normalizes_bilingual_paper_annotation_tags(self) -> None:
        annotation = normalize_paper_annotation({
            "title": "Deep Residual Learning for Image Recognition",
            "aliases": [" ResNet ", ""],
            "tags": {
                "zh": [" 卷积神经网络 ", "", "残差连接"],
                "en": [" CNN ", "image classification"],
            },
        })
    ...
    def test_legacy_annotation_tags_are_not_preserved(self) -> None:
        annotation = normalize_paper_annotation({
            "title": "Some Paper",
            "aliases": [],
            "tags": ["legacy tag"],
        ...
        self.assertEqual(annotation["tags"], {"zh": [], "en": []})

    def test_upsert_paper_annotation_initializes_bilingual_tags(self) -> None:
        annotations: dict[str, dict] = {}

        upsert_paper_annotation(annotations, "hash", "Some Paper")

        self.assertEqual(annotations["hash"], {
            "title": "Some Paper",
            "aliases": [],
            "tags": {"zh": [], "en": []},
        })
    ...
    def test_run_ingest_normalizes_external_venue_before_writing(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            settings = Settings.load(root)
            settings.pdf_dir.mkdir(parents=True)
            settings.mineru_output_dir.mkdir(parents=True)
            settings.paper_data_dir.mkdir(parents=True)
            write_venue_aliases(settings.data_dir / "venue_aliases.json")
            pdf_path = settings.pdf_dir / "Some_Paper.pdf"
            pdf_path.write_bytes(b"%PDF-1.4 fake")
            file_hash = sha256_file(pdf_path)
            mineru_dir = settings.mineru_output_dir / "Some_Paper"
